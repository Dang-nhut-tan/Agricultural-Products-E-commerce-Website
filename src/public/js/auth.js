...
if (authPath === "/dang-nhap" || authPath === "/dang-ky") {
  ...
  document.querySelector("#authForm").addEventListener("submit", async (event) => {
    ...
    if (!isLogin && payload.password !== payload.passwordConfirmation) { errorBox.textContent = "Mật khẩu xác nhận không khớp."; button.disabled = false; return; }
    try {
      const response = await fetch(`/api/auth/${isLogin ? "login" : "register"}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
...
fetch("/api/auth/me").then((response) => response.ok ? response.json() : null).then((result) => {
  ...
  document.querySelectorAll(".modern-account").forEach((link) => { link.href = "/tai-khoan"; const label = link.querySelector("b"); if (label) label.textContent = result.data.name || "Tài khoản"; });
  if (authPath === "/tai-khoan") authMain.innerHTML = `<section class="page-shell"><div class="container"><div class="panel"><h1>Xin chào, ${String(result.data.name || "bạn").replace(/[&<>"']/g, "")}</h1><p>${result.data.email}</p><button id="logoutButton" class="outline">Đăng xuất</button></div></div></section>`;
  document.querySelector("#logoutButton")?.addEventListener("click", async () => { await fetch("/api/auth/logout", { method: "POST" }); location.href = "/dang-nhap"; });
});
