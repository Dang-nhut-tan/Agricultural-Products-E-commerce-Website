...
if(location.pathname==='/tin-tuc'||newsDetailMatch){
  ...
  if(newsDetailMatch){
    fetch('/api/news/'+newsDetailMatch[1]).then(response=>{if(!response.ok)throw new Error();return response.json()}).then(({data:item})=>{
    ...
    }).catch(()=>{main.innerHTML='<div class="clean-empty"><h2>Không thể tải tin tức</h2></div>'});
  }
}
